...
const Filtros_Ordenamientos = () => {
  ...
  const onCheck = (e) => {
    if (e.target.checked) {
      !seleccionado.includes(e.target.name) &&
        setSeleccionado([...seleccionado, e.target.name]);
    } else {
      setSeleccionado(seleccionado.filter((elem) => elem !== e.target.name));
    }
  };
    ...
    <>
      <div className={estilos.container}>
        <div className={estilos.contenedor_elementos}>
          <p className={estilos.productos}>
            {categoriaobusqueda?.split("category=")[1] || categoriaobusqueda}
          </p>
          <p className={estilos.resultados}>
            {filtrados.length || searched_products.length}{" "}
            <label>Resultados</label>
          </p>
          <label className={estilos.ordenar_por}>
            Ordenar por:
          ...
          <form onSubmit={(e) => handleOnSubmit(e)}>
            ...
            <input
              ...
              required
            ></input>
            <div className={estilos.button_contenedor}>
